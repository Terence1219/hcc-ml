...
class YOLOLayer(nn.Module):
    ...
    def compute_grid_offsets(self, grid_size, device):
        ...
        self.stride = self.img_dim / self.grid_size   # e.g., 416 / 13 = 32
        # Calculate offsets for each grid
        grid_y, grid_x = torch.meshgrid(
            torch.arange(self.grid_size), torch.arange(self.grid_size))
        grid_x = grid_x.float().to(device)      # [grid_size, grid_size]
        grid_y = grid_y.float().to(device)      # [grid_size, grid_size]
        '''
        Checkpoint 1. Rewrite following 2 lines using torch.unsqueeze
        '''
        self.grid_x = grid_x[None, None, :, :]  # [1, 1, grid_size, grid_size]
        self.grid_y = grid_y[None, None, :, :]  # [1, 1, grid_size, grid_size]
        self.scaled_anchors = torch.tensor([
            [a_w / self.stride, a_h / self.stride]
            for a_w, a_h in self.anchors
        ]).to(device)
        self.anchor_w, self.anchor_h = self.scaled_anchors.T  # [num_anchors]
        '''
        Checkpoint 2. Rewrite following 2 lines by index tensor with `None`
        '''
        self.anchor_w = self.anchor_w.view((1, self.num_anchors, 1, 1))
        # [1, 1, num_anchors, 1]
        self.anchor_h = self.anchor_h.view((1, self.num_anchors, 1, 1))
        # [1, 1, num_anchors, 1]
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        # [batch, anchors_num, grid_num, grid_num]
        t_x = prediction[..., 0]
        # [batch, anchors_num, grid_num, grid_num]
        t_y = prediction[..., 1]
        # [batch, anchors_num, grid_num, grid_num]
        ...
        # [batch, anchors_num, grid_num, class_num]
        ...
        with torch.no_grad():
            '''
            Checkpoint 3. implement page 13
            Hint: sigma is sigmoid function
            '''
            pred_x = torch.???(t_x) + self.grid_x
            pred_y = torch.???(t_y) + self.grid_y
            pred_w = torch.???(t_w) * self.anchor_w
            pred_h = torch.???(t_h) * self.anchor_h
            pred_boxes = torch.stack([pred_x, pred_y, pred_w, pred_h], dim=-1)

        output = torch.cat(
            (
                pred_boxes.view(num_samples, -1, 4) * self.stride,
                pred_conf.view(num_samples, -1, 1).detach(),
                pred_cls.view(num_samples, -1, self.num_classes).detach(),
            ),
            -1,
        )
